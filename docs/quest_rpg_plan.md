# Quest RPG: Envanter ve Eşya Sistemi Planı

## Hedef
Quest RPG oyun döngüsüne derinlik katmak amacıyla oyuncuların silah, zırh ve iksir satın alabileceği, kuşanabileceği ve savaşlarda kullanabileceği bir Envanter Sistemi geliştirmek.

## Kullanıcı Onayı Gereken Konular
- Silahlar, Zırhlar ve İksirler için belirlediğim fiyat/hasar dengesi BBS'nin zorluk derecesine uygun mu? Değişiklik isterseniz belirtebilirsiniz.
- Veritabanını karmaşıklaştırmamak adına eşyaları ayrı bir `inventory` tablosu yerine doğrudan kullanıcının `player_stats` tablosunda basitçe tutacağım (örn: `weapon = 2`, `armor = 1`, `potions = 5`). Bu BBS mantığına çok daha uygundur, onaylıyor musunuz?

## Önerilen Değişiklikler

### 1. Veritabanı Güncellemesi (`database/schema.sql` & `server.js`)
- `player_stats` tablosuna aşağıdaki kolonlar eklenecek:
  - `weapon` INTEGER DEFAULT 0 (Kuşanılmış silah ID'si)
  - `armor` INTEGER DEFAULT 0 (Kuşanılmış zırh ID'si)
  - `potion_s` INTEGER DEFAULT 0 (Küçük İksir sayısı)
  - `potion_m` INTEGER DEFAULT 0 (Orta İksir sayısı)
  - `potion_l` INTEGER DEFAULT 0 (Büyük İksir sayısı)

### 2. Eşya Veri Yapıları (`server.js`)
- Genişletilmiş ve RPG dinamiklerine uygun 10 Silah, 10 Zırh ve 3 İksir eklenecek:
  ```javascript
  const WEAPONS = [
  ...
  const POTIONS = [
      { id: 's', name: "Küçük İksir", heal: 25, price: 20 },
      { id: 'm', name: "Orta İksir", heal: 75, price: 50 },
      { id: 'l', name: "Büyük İksir", heal: 200, price: 120 }
  ];
...
### 3. Dükkan Revizyonu (`Kanlı Demirci`)
- Dükkan menüsü genişletilecek (`STATES.GAME_SHOP`):
  - `[1] Maksimum Can +5` (Eski sistem devam)
  - `[2] Silah Pazarı` (Yeni Alt Menü: `STATES.GAME_SHOP_WEAPONS`)
  - `[3] Zırh Tüccarı` (Yeni Alt Menü: `STATES.GAME_SHOP_ARMORS`)
  - `[4] İksir Dükkanı` (Yeni Alt Menü: `STATES.GAME_SHOP_POTIONS`)

### 4. Envanter Arayüzü (`[I] Envanter`)
- Oyun ana menüsüne `[E] Envanter ve Durum` eklenecek.
- Bu ekranda oyuncunun Can, Stamina, Altın, Seviye, Kuşandığı Silah, Kuşandığı Zırh ve İksir sayısı görüntülenecek.
- `[P] İksir İç` komutuyla doğrudan can yenileme yapılabilecek.

### 5. Savaş (Combat) Sistemi Güncellemesi
- `handleAttack` fonksiyonu güncellenecek:
  - Oyuncu Hasarı = Temel Hasar (5-10) + `WEAPONS[weapon].damage`
  - Düşman Hasarı = Düşman Temel Hasarı - `ARMORS[armor].defense` (Minimum hasar 0 olacak şekilde).
- Savaş esnasında menüye `[P] İksir İç` eklenecek. Böylece oyuncular savaşta ölmek üzereyken iksir kullanabilecek.

## Doğrulama Planı
1. Ormanda canavarlarla savaşıp altın toplayacağım.
2. Dükkana gidip `Paslı Kama` ve `Deri Zırh` satın alacağım.
3. Envanter (`E`) menüsünden eşyalarımı kontrol edeceğim.
4. Savaşa tekrar girip hasarımın arttığını ve aldığım hasarın düştüğünü teyit edeceğim.
5. İksir satın alıp savaş ortasında kullanmayı deneyeceğim.
